...
class Yud_public extends CI_Controller {
	...
	function get_data_akun2($format = 'json'){
		$api_search = $this->input->post('api_search');
		$q 			= $this->mdl_00->get_data_akun2($api_search[0]);
	...
	function get_rekap_nilai_ujian($format = 'json'){
		$api_search = $this->input->post('api_search');
		$q 			= $this->mdl_00->get_rekap_nilai_ujian($api_search[0], $api_search[1], $api_search[2]);

		$this->sia_api_lib_format->output($q, $format);
	}


	function get_data_peserta2($format = 'json'){
		$api_search = $this->input->post('api_search');
		$q 			= $this->mdl_00->get_data_peserta2($api_search[0], $api_search[1], $api_search[2], $api_search[3]);
