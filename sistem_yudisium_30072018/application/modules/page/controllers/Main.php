...
class Main extends CI_Controller {
    ...
    function menu_json(){
        ...
        $this->datatables->select("m1.id_menu as id_menu, m1.nama_menu as nama_menu, m1.url as url, m1.jenis_url as jenis_url, m1.target as target, m1.parent as parent")
	        ->add_column('action', '$1', 'm1.id_menu')
	        ->from("menu as m1")
			->join('menu as m2', 'm1.parent=m2.id_menu', 'left')
			->where('m1.kode_unit', $kode_unit);
        
        echo $this->datatables->generate();
    }
	function menu_json_page(){
		 $kode_unit = $this->session->userdata('kode_unit');
        $this->datatables->select("id_page, title, foto, content, kode_bahasa")
        ->add_column('action', '$1', 'id_page')
        ->from("page")->where('kode_unit', $kode_unit);
        echo $this->datatables->generate();
    }
	...
	function edit($id){
		if($_POST==NULL) {
			...
			$data['parent']=$this->db->get_where('menu',array('parent'=>0, 'kode_unit'=>$kode_unit))->result();
			$data['target']=$this->db->get_where('menu',array('id_menu'=>$id))->row_array();
    ...
    // FAUZI 20/06/18 START
    public function panduan(){
		$data['title']="Panduan";
		$data['main_view']="admin/panduan";
		$this->load->view('admin/content',$data);
    }
    // FAUZI 20/06/18 END
