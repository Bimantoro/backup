...
class Yud_basic extends CI_Model {
	...
	function up_cfg_pilihan($tahun, $jalur, $gel, $kelas, $pilihan, $mulai, $selesai, $no, $ketua, $tgl){
		return $q = $this->yud->query("UPDATE tb_config_pilihan SET tgl_mulai = '$mulai', tgl_selesai = '$selesai', ketua_yudisium = '$ketua', tgl_yudisium = '$tgl', no_yudisium = '$no', kode_kelas = '$kelas', pilihan = '$pilihan' WHERE tahun = '$tahun' AND kode_jalur = '$jalur' AND kode_gelombang = '$gel'");
	}
	...
	function get_data_yudisium2($prodi, $tahun, $gelombang, $jalur, $pilihan, $kelas){
		...
		) t3 ON t1.nomor_peserta = t3.nomor_peserta WHERE t1.kode_prodi = '$prodi' AND t1.pilihan = '$pilihan' AND kode_kelas = '$kelas' AND t1.nomor_peserta NOT IN (SELECT nomor_peserta FROM tb_yudisium WHERE pilihan = '$oposite' AND status = '1') ORDER BY COALESCE(t3.nilai_total,0) DESC";
	...
	function get_detail_nilai_ujian($peserta){
		$q = $this->yud->query("SELECT t1.nomor_peserta AS nomor_peserta, t1.nilai AS nilai, t2.nama_ujian AS nama_ujian, t3.nama_peserta AS nama_peserta, t3.asal_sekolah AS asal_sekolah, t4.nama_prodi as pilihan1, t5.nama_prodi as pilihan2
		...
		if(empty($q)){
			$q = $this->yud->query("SELECT t1.nomor_peserta as nomor_peserta, t1.nama_peserta as nama_peserta, 'X0X' as nilai, 'X0X' AS nama_ujian, t1.asal_sekolah as asal_sekolah, t2.nama_prodi as pilihan1, t3.nama_prodi as pilihan2 
	...
	function get_data_peserta_diterima($tahun, $gelombang, $jalur, $kelas, $kd_prodi){
		$sql = "SELECT t1.nomor_peserta AS nomor_peserta, t2.nama_peserta AS nama_peserta FROM tb_yudisium t1 LEFT JOIN tb_data_peserta t2 ON t1.nomor_peserta = t2.nomor_peserta WHERE tahun = '$tahun' AND kode_gelombang = '$gelombang' AND kode_jalur = '$jalur' AND kode_kelas = '$kelas' AND kode_prodi = '$kd_prodi' AND status = '1'";
