...
class Pengumuman extends CI_Controller {
   ...
   public function index($uri=0){
	...
	function menu_json(){
        ...
        $this->datatables->select("m1.id_pengumuman id_pengumuman,m1.nama_pengumuman nama_pengumuman,concat(substring(m1.nama_file,1, 25),'...') as nama_file,m1.url url,m1.tgl_posting tgl_posting")
	...
	function add_pengumuman(){
		if($_POST==null){
			...
			if ($_FILES['nama_file']['size']>=10000000) {
				$this->session->set_flashdata('error','File tidak boleh kosong atau lebih dari 2 MB!');
				redirect('admin/pengumuman/add_pengumuman');
			}else{
				move_uploaded_file($_FILES["nama_file"]["tmp_name"],"./media/pengumuman/".$filename);
				$data=array(
							'kode_unit'=>$kode_unit,
	...
	function edit($id=""){
		if($_POST==NULL) {
			...
			$this->load->view('admin/content',$data);
				...
				if($filename != null){
					if ($_FILES['nama_file']['size']>=2000000) {
					$this->session->set_flashdata('error','File tidak boleh kosong atau lebih dari 2 MB!');
