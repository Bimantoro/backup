...
class pengumuman extends CI_Controller {
   ...
   public function index($uri=0,$th="",$bl=""){
		...
		$this->breadcrumb->append_crumb(ucfirst(dict('Pengumuman',$this->lang)), '/');
	...
	public function detail($id=0){
			...
			$this->breadcrumb->append_crumb(ucfirst(dict('Beranda',$this->lang)), base_url($this->lang));
			$this->breadcrumb->append_crumb(ucfirst(dict('Pengumuman',$this->lang)), site_url($this->lang.'/pengumuman'));
			$this->breadcrumb->append_crumb(substr($p->nama_pengumuman,0,130).' ...', '/');
			$data['title']=$p->nama_pengumuman;
			$data['gambar'] = $p->nama_file;

			$icon = array('pdf', 'zip', 'rar', 'xls', 'xlsx', 'doc', 'docx');
			$extension = explode(".", $p->nama_file);
			$ex = count($extension) - 1;
			if (in_array($extension[$ex], $icon)) {
				$data['icon'] = base_url('asset/images/icon/'.$extension[$ex].'.png');
			}
			
			$data['folder'] = "pengumuman";
			$arr_filter=array();
			$arr_filter=related_text($p->nama_pengumuman);	
			$filter	="id_pengumuman <> '".$id."' AND  (nama_pengumuman LIKE '%".rtrim(implode("%' OR nama_pengumuman LIKE '%",$arr_filter)," OR judul LIKE '%")."%')";
			//$data['rec']=$this->db->query("SELECT id_pengumuman,nama_pengumuman,tgl_posting from pengumuman WHERE ".$filter." ORDER BY tgl_posting desc LIMIT 0,5")->result();
			...
			$data['rec']=$this->page_model->get_pengumuman_terkait($filter, 0, 5);
			$data['pop']=$this->page_model->get_pengumuman_terpopuler(0, 5);
			...
			if($get_data->num_rows()>0){
				set_time_limit(0); 
				if($p->url!=null && $p->nama_file==null){
					/* redirect($p->url); */
					// header('Location: '.$p->url);
					$string = array('http://', 'https://');
					$url = str_replace($string, '', $p->url);
					$url = 'http://'.$url;
					 header('Location: '.$url);
				}else{
					...
					$arr_ext=array('pdf','jpg', 'jpeg','png');
	...
	function download($id=0){
	   ...
	   function feed(){  
			$data['feed_name'] = ucfirst(dict('Pengumuman',$this->lang));  
			$data['encoding'] = 'utf-8';  
