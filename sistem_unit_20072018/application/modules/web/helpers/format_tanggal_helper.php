...
if ( ! function_exists('tanggal_indonesia'))
{
	...
	{	
		...
		$bulan = substr(bulan_en($pecah[1]), 0, 3);	
		}else{
		$bulan = bulan($pecah[1]);
...
function tgl_periode($tgl1,$tgl2){
	...
	if($lang == 'en'){
		if($th1==$th2){
			if($bl1==$bl2){
				if($hr1==$hr2){
					return (int)$hr1.' '.bulan_en($bl1).' '.$th1;
				}else{
					return (int)$hr1.' - '.(int)$hr2.' '.bulan_en($bl2).' '.$th2;
				}
			}else{
				return (int)$hr1.' '.bulan_en($bl1).' - '.(int)$hr2.' '.bulan_en($bl2).' '.$th2;
			}
		}else{
			return (int)$hr1.' '.bulan_en($bl1).' '.$th1.' - '.(int)$hr2.' '.bulan_en($bl2).' '.$th2;
		}
	}else{
		if($th1==$th2){
			if($bl1==$bl2){
