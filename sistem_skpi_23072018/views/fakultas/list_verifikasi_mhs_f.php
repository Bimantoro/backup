					...
					if(mhs.length > 0){
							...
							$.each(mhs, function(index, value){
								...
								if(value.STATUS == 1){
									...
									row += "<td>";
									row += "<button class='btn-uin btn-inverse btn-small' type='submit' name='nimx' value='"+value.NIM+"'>Draft</button>";
									row += "</td>";
								}else{
...
<div>		
	<ul id="crumbs">
		...
		<li>Data Pendaftar <?php echo $string_skpi[0]; ?></li>
	</ul>
</div>
...
<div id="konten" hidden>
	<h3>Data Pendaftar Verifikasi <?php echo $string_skpi[0]; ?></h3>
	...
	<form action="<?php echo base_url(); ?>skpi/skpi_fakultas/cetak_draft_skpi" method="POST" target="_blank">
