...
<body>
	<br>
	<form class="form-inline" action="<?php echo base_url(); ?>skpi/skpi_admin/status_skpi_mhs" method="POST">
	...
	<div id="konten">
		<?php if(isset($mhs)){ ?>
		<h3>Ubah Status SKPI Mahasiswa</h3>
		<?php if(count($mhs)<=0){ ?>
			<div>
				<div class="bs-callout bs-callout-error">Tidak ada data untuk ditampilkan !</div>
			</div>
		<?php }else{ ?>
		<form action="<?php echo base_url(); ?>skpi/skpi_admin/skpi_mhs" method="POST">
			...
			<?php if($key['STATUS']){ ?>
				<tr>
					...
					<td align="center" style="vertical-align: middle;">
						<button class="btn-uin btn-inverse btn-small" type="submit" onclick="return confirm('Apakah anda yakin melakukan perubahan status SKPI ?');" name="btn_status" value="<?php echo $key['NIM']; ?>" >Ubah</button>
					</td>
					<?php } ?>
				</tr>
			<?php
