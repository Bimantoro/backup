...
function get_agenda($id)
	...
	function get_units(){
		$this->db->from("unit");
		$this->db->where(array('kode_bahasa' => 'id'));
		$this->db->order_by("nama_unit", "asc");
		return $this->db->get()->result();
		//return  $this->db->get_where("unit", array('kode_bahasa' => 'id'))->result();
	}
	...
	function get_unit_by_id($id){
		$q = "SELECT * FROM unit WHERE id='$id'";
        $query = $this->db->query($q);
        return $query->result();
	...
	function get_kamus($id=''){
		$q = $this->db->query("SELECT * FROM kamus WHERE id_kamus = '$id'");
		return $q->result();
	}

	function cek_kamus($kata='', $lang=''){
		//$q = $this->db->query("SELECT * FROM kamus WHERE kode_bahasa = '$lang' AND kata = '$kata'");
		$q = $this->db->get_where('kamus', array('kode_bahasa' => $lang, 'kata' => $kata));
		return $q->row_array();
	}

	function cek_unit_by_subdomain($subdomain){
		$q = $this->db->query("SELECT * FROM unit WHERE subdomain = '$subdomain'");
		return $q->result_array();
	}
	...
	function get_last_insert_unit($kd_unit, $lang, $subdomain){
		return $this->db->get_where('unit', array('kode_unit' => $kd_unit, 'kode_bahasa' => $lang, 'subdomain' => $subdomain))->row_array();
	}

	
	 //TONI EFENDI 11/7/2018 <START>
	function get_data_page($kode_unit){
		$query = "SELECT id_page, title FROM page WHERE kode_unit='$kode_unit' ORDER BY id_page DESC ";
		$sql = $this->db->query($query);
		return $sql->result_array();
	}
	function get_data_page_by_id($id_page){
		$query = "SELECT * FROM menu WHERE id_menu = '$id_page'";
		$sql = $this->db->query($query);
		return $sql->row_array();
	}
	function get_target_menu($id_menu){
		$query = "SELECT * FROM menu WHERE id_menu = '$id_menu'";
		$sql = $this->db->query($query);
		$data = $sql->row_array();
		return $data['target'];
	}
	function get_jenisurl_menu($id_menu){
		$query = "SELECT * FROM menu WHERE id_menu = '$id_menu'";
		$sql = $this->db->query($query);
		$data = $sql->row_array();
		return $data['jenis_url'];
	}
