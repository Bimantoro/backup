...
class User extends CI_Controller {
   ...
   public function index($uri=0){
	...
	function menu_json(){
        ...
        //  $kode_unit = $this->session->userdata('kode_unit');
        // $this->datatables->select("user.username, user.kode_unit, unit.nama_unit, user.level")
        // ->add_column('action', '$1','user.username')
        // ->from("user")->join('unit', 'unit.kode_unit=user.kode_unit');

         $kode_unit = $this->session->userdata('kode_unit');
        $this->datatables->select("m1.username as username, m1.kode_unit as kode_unit, m2.nama_unit as nama_unit, m1.level as level")
