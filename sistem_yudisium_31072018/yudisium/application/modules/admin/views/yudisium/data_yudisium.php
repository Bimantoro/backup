...
<script>
	...
	function fill_data(){
		...
		$.ajax({
			...
			type : 'POST',
			data : 'prodi='+prodi+'&tahun='+tahun+'&gelombang='+gelombang+'&jalur='+jalur+'&pilihan='+pilihan+'&kelas='+kelas,
	...
	function show_jml_diterima_awal(){
		$('#f-diterima').html(jumlah_diterima);
		console.log(jumlah_diterima);
	}
	...
	function sv_diterima_all(){
		var prodi = prodi_aktif;
		var pilihan = pilihan_aktif;
		var kelas = kelas_aktif;

		var status = '0';
		//cek apakah centang atau sebaliknya
		if($('.diterima:checked').length == $('.diterima').length){
			status = '1';
		}

		$.ajax({
			url 	: '<?php echo $json_url_diterima_prodi ?>',
			type 	: 'POST',
			data 	: 'prodi='+prodi+'&pilihan='+pilihan+'&status='+status+'&kelas='+kelas,
